...
void* fake_dlopen_ext(char* libPath, int mode, void* extInfo){
    ...
    if(strstr(libPath, "libdu.so")) {
        ...
        void* query_internal =  pointer_add(du_handle->load_addr, 0x21d71);
...
void my_signal_handler (int sig, struct siginfo* si, void* data) {
    logd(TAG, "segv occured, code: %d", si->si_code);
}
